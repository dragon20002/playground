...
2. Write 요청은 Read/Write DB로 직접 보내고, Read 요청은 L4 스위치를 거쳐 적절히 나눠 보낸다.
...
변경 데이터를 추출한 다음에는 내장된 Streaming 기능이나 Redis, Apache Kafka 등의 별도 메시징 솔루션을 사용하여 다른 데이터베이스에 변경 데이터를 보내 동기화한다.
